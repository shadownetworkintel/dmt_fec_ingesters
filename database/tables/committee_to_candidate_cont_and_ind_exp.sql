CREATE TABLE committee_to_candidate_cont_and_ind_exp (
	...
	amndt_ind        char(1),       -- Indicates if the report being filed is new (N), an amendment (A) to a previous report or a termination (T) report.
	rpt_tp           varchar(3),    -- report type see below
	transaction_pgi  varchar(5),    -- Primary/general indicator This code indicates the election for which the contribution was made. EYYYY (election Primary, General, Other plus election year)
	image_num        varchar(18),   -- image number
	transaction_tp   varchar(3),    -- transaction type see below
...
CREATE INDEX idx_committee_to_candidate_tran_id
...
-- The contributions from committees to candidates file is a subset of the itemized records 
-- (OTH) file and contains each contribution or independent expenditure made by a:
-- PAC
-- Party committee
-- Candidate committee
...
-- committee receiving the money, the amount given, and the date it was given, and 
-- other information. 
