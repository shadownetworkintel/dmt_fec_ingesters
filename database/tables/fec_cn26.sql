CREATE TABLE fec_cn26 (
    ...
    cand_office             CHAR(1) CHECK (cand_office IN ('H', 'S', 'P')), -- House (H), Senate (S), President (P)
    cand_office_district    VARCHAR(2), -- Congressional district (House races only, "00" for Senate)
    cand_ici                CHAR(1) CHECK (cand_ici IN ('I', 'C', 'O')), -- Incumbent (I), Challenger (C), Open (O)
    cand_status             CHAR(1) CHECK (cand_status IN ('C', 'F', 'N', 'P')), -- Current (C), Future (F), Not running (N), Prior (P)
