...
class TestRunAllIngesters:
    ...
    def test_run_all_ingesters_success(self):
        ...
        with patch('ingesters.api_candidates_new.main') as mock_candidates, \
             patch('ingesters.api_committees_new.main') as mock_committees, \
    ...
    def test_ingester_execution_order(self):
        ...
        with patch('ingesters.api_committees_new.main', side_effect=track_call('committees')), \
             patch('ingesters.api_candidates_new.main', side_effect=track_call('candidates')), \
             ...
             patch('ingesters.api_totals_ingester.main', side_effect=track_call('totals')):

            from scripts.run_all_ingesters import main
            main()

            # Verify execution order matches run_all_ingesters
            expected_order = [
                'committees',
                'candidates',
                'schedule_a',
    ...
    def test_success_logging(self):
        ...
        with patch('ingesters.api_candidates_new.main') as mock_candidates, \
             ...
             patch('ingesters.api_schedule_b_ingester.main') as mock_schedule_b, \
             patch('ingesters.api_schedule_e_ingester.main') as mock_schedule_e, \
             patch('ingesters.api_totals_ingester.main') as mock_totals, \
             patch('scripts.run_all_ingesters.logger') as mock_logger:
            ...
            success_calls = [
                ...
                if "completed successfully" in call
            ]
            # All 6 ingesters (committees, candidates, A, B, E, totals) succeeded
            assert len(success_calls) == 6

    def test_pipeline_start_end_logging(self):
        """Test that pipeline start and end are logged."""
        with patch('ingesters.api_candidates_new.main'), \
             patch('ingesters.api_committees_new.main'), \
    ...
    def test_exception_handling_continues_execution(self):
        ...
        with patch('ingesters.api_candidates_new.main'), \
             patch('ingesters.api_committees_new.main', side_effect=count_calls), \
             patch('ingesters.api_schedule_a_ingester.main') as mock_schedule_a, \
             patch('ingesters.api_schedule_b_ingester.main') as mock_schedule_b, \
             patch('ingesters.api_schedule_e_ingester.main') as mock_schedule_e:
...
class TestRunAllIngestersRobustness:
    ...
    def test_import_errors(self):
        ...
        with patch('ingesters.api_candidates_new.main', side_effect=ImportError("Module not found")), \
             patch('ingesters.api_committees_new.main'), \
             patch('ingesters.api_schedule_a_ingester.main'), \
             patch('ingesters.api_schedule_b_ingester.main'), \
             patch('ingesters.api_schedule_e_ingester.main'):
    ...
    def test_different_exception_types(self):
        ...
        with patch('ingesters.api_candidates_new.main', side_effect=ValueError("Invalid value")), \
             patch('ingesters.api_committees_new.main', side_effect=ConnectionError("Network error")), \
    ...
    def test_none_return_values(self):
        ...
        with patch('ingesters.api_candidates_new.main', return_value=None), \
             patch('ingesters.api_committees_new.main', return_value=None), \
